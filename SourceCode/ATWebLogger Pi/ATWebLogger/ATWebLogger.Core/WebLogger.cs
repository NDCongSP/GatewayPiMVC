...
namespace ATWebLogger.Core
{
    public class WebLogger
    {
        ...
        public WebLogger()
        {
            //EmailFromInfo = JsonConvert.DeserializeObject<FromEmailInfo>(File.ReadAllText($"{PathFile}EmailFromInfo.json"));

            string[] _data = File.ReadAllLines($"{PathFile}EmailFromInfo.txt");
            EmailFromInfo.UserEmailFrom = _data[0];
            ...
            SMSString = ReadText(PathFile + "sms.txt").Trim();
            EmailString = ReadText(PathFile + "email.txt").Trim();
            //Password = "admin";
            //SMSString = "0767397108,‭0795864635,0909167655";
            //SMSString = "0909167655,‭0909167655,0909167655";
            //EmailString = "ndcong08cddv02@gmail.com";
            Console.WriteLine($"sms {SMSString} | Email {EmailString}");
                        ...
                        if (location.State == "Enable")
                        {
                            ...
                            if (locationAlarm.OnOff == 1 && locationAlarm.OnOffFlag == false)
                            {
                                ...
                                if (WriteValue(writeData.DeviceId, writeData.Address, writeData.DataType, writeData.Value) == "Ok")
                                {
                                    ...
                                    Console.WriteLine($"On serien alarm successful.");
                                }
                                else
                                {
                                    Console.WriteLine($"On serien alarm fail.");
                                }
                            ...
                            else if (locationAlarm.OnOff == 0 && locationAlarm.OnOffFlag == true)
                            {
                                ...
                                if (_countNormal == _onOffSirenAlarm.Count)
                                {
                                    ...
                                    {
                                        locationAlarm.OnOffFlag = true;
                                        Console.WriteLine($"Off serien alarm successful.");
                                    }
                                    else
                                    {
                                        Console.WriteLine($"Off serien alarm fail.");
                                    }
        ...
        private void ReadAlarmLocations()
        {
            ...
            if (WriteValue(writeData.DeviceId, writeData.Address, writeData.DataType, writeData.Value) == "Ok")
            {
                Console.WriteLine($"Off serien when startup successful.");
            }
            else
            {
                Console.WriteLine($"Off serien when startup fail.");
            }
                    ...
                    try
                    {
                        if (location.State == "Enable" && !string.IsNullOrEmpty(location.Value) && location.Value != "null" && location.Status == "Good")
                        {
                            ...
                            if (double.TryParse(location.Value, out double value))
                            {
                                ...
                                {
                                    //Console.WriteLine($"Hight Coutn Alarm {location.CountHighAlarm}");
                                ...
                                {
                                    //Console.WriteLine($"Low Coutn Alarm {location.CountLowAlarm}");
                                ...
                                {
                                    //Console.WriteLine($"Normal Coutn Alarm {location.CountNormalAlarm}");
        ...
        public void SendAlarmEmail(string alarm, Location location)
        {
            try
            {
                GateWay.Email.CredentialEmail = EmailFromInfo.UserEmailFrom;// "giamsat.canhbao@gmail.com";
                GateWay.Email.CredentialPass = EmailFromInfo.PassEmailFrom;// "ujgknqghwcjqxfvq";

                GateWay.Email.emailTo = EmailString;
                GateWay.Email.subjectEmail = EmailFromInfo.Subject;
                ...
                Console.WriteLine($"EMAIL {strAlarm}");
        ...
        public void SendAlarmSMS(string alarm, Location location)
        {
            try
            {
                ...
                //    Console.WriteLine($"Gui SMS {GateWay.SMS.GuiSMS(SMSString.Split(',')[j], noidung)}");
                //    Thread.Sleep(2000);
                //}
        ...
        {
            //Save email string
            WriteText($"{EmailString.Trim()}", PathFile + "email.txt");
            //Save sms string
            WriteText($"{SMSString.Trim()}", PathFile + "sms.txt");
